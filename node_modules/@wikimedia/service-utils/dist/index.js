...
var _yargs2 = require('yargs'); var _yargs3 = _interopRequireDefault(_yargs2);
...
var WikimediaEcsFormatMiddleware = class {
  transform(info) {
    ...
    requestIdInfo[_triplebeam.LEVEL] = info[_triplebeam.LEVEL];
    requestIdInfo[_triplebeam.MESSAGE] = info[_triplebeam.MESSAGE];
    requestIdInfo[_triplebeam.SPLAT] = info[_triplebeam.SPLAT];
  ...
  processServiceName(info) {
    ...
    const serviceName = _nullishCoalesce(info.serviceName, () => ( info.service));
  ...
  processHttpData(info) {
    ...
    if (info.request) {
      ...
      url.path = _nullishCoalesce(_nullishCoalesce(((_a = requestData.params) == null ? void 0 : _a[0]), () => ( requestData.path)), () => ( ""));
    ...
    return _defu.defu.call(void 0, baseInfo, {
      http,
      url,
      source,
  ...
  processRequestId(info) {
    ...
    const requestId = _nullishCoalesce(info.requestId, () => ( info.request_id));
...
var createWinstonLogger = (serviceName, options = {}) => {
  ...
  const ecsFormatter = [
    _ecswinstonformat.ecsFields.call(void 0, { convertReqRes: true, ...options.formatOptions }),
    wikimediaEcsFormatMiddleware(),
    _ecswinstonformat.ecsStringify.call(void 0, )
  ];
  return _winston2.default.createLogger({
    ...
    format: _winston2.default.format.combine(
      ...[
        ...
        _winston2.default.format.errors({ stack: options.stacktrace }),
        options.format === "ecs" ? ecsFormatter : _winston2.default.format[_nullishCoalesce(options.format, () => ( "json"))](
    ...
    transports: (_nullishCoalesce(options.transports, () => ( []))).map(
      ({ transport, options: transportOptions }) => (
        // @ts-ignore
        new _winston2.default.transports[transport]({
          ...transportOptions,
          ...
          ...(transportOptions == null ? void 0 : transportOptions.format) ? {
            format: _winston2.default.format.combine(
              ...[
                _winston2.default.format.errors({ stack: options.stacktrace }),
                transportOptions.format === "ecs" ? ecsFormatter : _winston2.default.format[transportOptions.format](
...
// src/index.ts
var _promclient = require('prom-client'); var _promclient2 = _interopRequireDefault(_promclient);
...
var PrometheusClient = class {
  ...
  constructor(logger) {
    this.logger = logger;
    this.client = _promclient2.default;
  }
  makeMetric(options) {
...
var Metrics = class {
  ...
  constructor(serviceName, logger, options) {
    ...
    logger.error(
      `No such metrics client: '${_nullishCoalesce(options.type, () => ( "undefined"))}'`
    );
  }
  getServiceName() {
...
var PrometheusServer = class {
  ...
  constructor(config, defaultLabels = { service: "default_service" }) {
    ...
    if (this.config.collectDefaultMetrics !== false) {
      _promclient2.default.collectDefaultMetrics();
    }
    _promclient2.default.register.setDefaultLabels(defaultLabels);
    this.server = _http2.default.createServer(async (req, res) => {
      res.end(await _promclient2.default.register.metrics());
    });
  }
  start() {
  ...
  async close() {
    _promclient2.default.register.clear();
...
var wrapRouteHandlers = (app, metrics, subPath = "") => {
  const router = _nullishCoalesce(app._router, () => ( app));
    ...
    res.end = (...args) => {
      ...
      if (req.subPath) {
        ...
        path2 = _replacePathWithPlaceholders(
          req.baseUrl + path2,
          Object.entries(_nullishCoalesce(req.params, () => ( [])))
        ).replace(/\/$/, "");
      }
      if (!path2 || !req.route) {
        path2 = _nullishCoalesce(req.subPath, () => ( "/"));
      }
      if (legacyPathNaming) {
...
var extractTraceContext = (headers) => {
  ...
  const newHeaders = new Headers({
    "x-request-id": _nullishCoalesce(_headers.get("x-request-id"), () => ( _uuid.v1.call(void 0, )))
  });
...
var XRequestPropagator = class {
  ...
  inject(context, carrier, setter) {
    ...
    setter.set(
      carrier,
      X_REQUEST_HEADER,
      _nullishCoalesce(((_b = (_a = _api.propagation.getBaggage(context)) == null ? void 0 : _a.getEntry(X_REQUEST_HEADER)) == null ? void 0 : _b.value), () => ( _uuid.v1.call(void 0, )))
    );
  }
  /**
  ...
  extract(context, carrier, getter) {
    ...
    let baggage = _nullishCoalesce(_api.propagation.getBaggage(context), () => ( _api.propagation.createBaggage()));
    baggage = baggage.setEntry(X_REQUEST_HEADER, { value: xRequestHeader });
    return _api.propagation.setBaggage(context, baggage);
  }
  fields() {
...
var _yargs = (...args) => (
  ...
  _yargs3.default.call(void 0, ...args).usage("Usage: $0 [command] [options]").options({
    "config-directory": {
      alias: "d",
...
_yargs(_helpers.hideBin.call(void 0, process.argv)).parseSync();
var ServiceUtils = class _ServiceUtils {
  ...
  constructor(config) {
    ...
    if (config.metrics) {
      this.prometheusServer = _ServiceUtils.createPrometheusServer(
        config.metrics,
        { service: _nullishCoalesce(config.service_name, () => ( "default_service")) }
      );
    }
  ...
  static createLogger(config) {
    return createWinstonLogger(
      _nullishCoalesce(config.service_name, () => ( "default_service")),
      config.logging
    );
  }
  ...
  static async loadConfig(options) {
    const parsedArgs = _yargs(_helpers.hideBin.call(void 0, process.argv)).parseSync();
    let configDirectory;
    let configFile;
    if ((options == null ? void 0 : options.useArgs) !== false) {
      if (parsedArgs.config) {
        const configPath = _path2.default.parse(parsedArgs.config);
        configDirectory = configPath.dir;
        configFile = configPath.name;
      }
      configFile ??= _nullishCoalesce(parsedArgs.configFile, () => ( (options == null ? void 0 : options.configFile)));
      configDirectory ??= _nullishCoalesce(_nullishCoalesce(parsedArgs.configDirectory, () => ( (options == null ? void 0 : options.configDirectory))), () => ( (options == null ? void 0 : options.cwd)));
      if (configDirectory && !/^\//.test(configDirectory)) {
        configDirectory = _path2.default.resolve(
          `${process.cwd()}/${_nullishCoalesce(configDirectory, () => ( ""))}`
        );
      }
    }
    const loadedConfig = await _c12.loadConfig.call(void 0, {
      name: "service-utils",
      ...options,
      ...{
        cwd: configDirectory,
...
var prometheus = _promclient2.default;
...
exports.ServiceUtils = ServiceUtils; exports.getInstance = getInstance; exports.helpers = helpers; exports.prometheus = prometheus; exports.teardown = teardown;
